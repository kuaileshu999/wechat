<template>
  <div class="page-container">
    <div class="page-header">
      <h2>学科管理</h2>
      <el-button v-if="userStore.hasPermission('subject:create')" type="primary" @click="openDialog()">新建学科</el-button>
    </div>
    <el-table :data="list" v-loading="loading" border stripe>
      ...
      <el-table-column label="操作" width="160">
        <template #default="{ row }">
          <el-button v-if="userStore.hasPermission('subject:create')" link type="primary" @click="openEdit(row)">编辑</el-button>
    ...
    <el-drawer v-model="dialogVisible" :title="editingId ? '编辑学科' : '新建学科'"
...
const { list, total, page, loading, load, onPageChange } = usePagination(
  params => request.get('/subjects', { params })
)
...
async function submit() {
  ...
  if (editingId.value) {
    await request.put(`/subjects/${editingId.value}`, { name: form.name.trim() })
  } else {
    await request.post('/subjects', { name: form.name.trim(), status: 1 })
  }
  showSuccess()
...
async function toggleStatus(row) {
  await request.put(`/subjects/${row.id}/status`, null, { params: { status: row.status === 1 ? 0 : 1 } })
  showSuccess()
  load()
}
